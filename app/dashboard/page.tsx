...
export default function DashboardPage() {
  ...
  function actualizarSugerencia(indice: number, cambios: Partial<SugerenciaIA>) {
  ...
  return (
    <div className="flex-1 px-4 py-10 sm:py-14">
      <div className="mx-auto w-full max-w-4xl space-y-10">
        <div>
          <h1 className="text-3xl font-semibold text-ink">Banco de preguntas</h1>
        ...
        <section className="m-card p-6 sm:p-8">
          <h2 className="text-lg font-semibold text-ink">Nueva pregunta (manual)</h2>
          ...
          <form onSubmit={handleSubmit} className="mt-5 space-y-4">
            ...
            <div className="pt-1">
              <button type="submit" disabled={guardando} className="m-btn m-btn-primary">
        ...
        <section className="m-card border-l-4 border-accent p-6 sm:p-8">
          ...
          <form onSubmit={handleGenerarIA} className="mt-5 space-y-4">
            ...
            <div className="pt-1">
              <button type="submit" disabled={generandoIA} className="m-btn m-btn-primary">
          ...
          {sugerenciasIA.length > 0 && (
            <div className="mt-7 space-y-4 border-t border-line pt-6">
              ...
              {sugerenciasIA.map((s, i) => (
                <div key={i} className="rounded-xl border border-line bg-elevated/60 p-4 sm:p-5">
                  <textarea
                    value={s.enunciado}
                    onChange={(e) =>
        ...
        <section>
          <h2 className="text-lg font-semibold text-ink">
            Tus preguntas <span className="font-normal text-muted">({preguntas.length})</span>
          </h2>
          {cargandoLista ? (
            <p className="mt-4 text-sm text-muted">Cargando...</p>
          ) : preguntas.length === 0 ? (
            <p className="mt-4 text-sm text-muted">Aún no tienes preguntas.</p>
          ) : (
            <ul className="mt-4 divide-y divide-line overflow-hidden rounded-xl border border-line">
              {preguntas.map((p) => (
                ...
                >
                  ...
                  {p.origen === "ia" && (
                    <span title="Generada por IA" className="shrink-0 text-accent">
                      ✨
                    </span>
                  )}
                </li>
